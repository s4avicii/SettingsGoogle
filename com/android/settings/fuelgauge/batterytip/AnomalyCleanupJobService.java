...
public class AnomalyCleanupJobService extends JobService {
    ...
    public static void scheduleCleanUp(Context context) {
        ...
        JobInfo.Builder persisted = new JobInfo.Builder(C0444R.integer.job_anomaly_clean_up, new ComponentName(context, AnomalyCleanupJobService.class)).setPeriodic(CLEAN_UP_FREQUENCY_MS).setRequiresDeviceIdle(true).setRequiresCharging(true).setPersisted(true);
        if (jobScheduler.getPendingJob(C0444R.integer.job_anomaly_clean_up) == null && jobScheduler.schedule(persisted.build()) != 1) {
