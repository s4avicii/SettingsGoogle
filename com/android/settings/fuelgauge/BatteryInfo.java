...
public class BatteryInfo {
    ...
    public void bindHistory(final UsageView usageView, BatteryDataParser... batteryDataParserArr) {
        ...
        C09251 r1 = new BatteryDataParser() {
            byte lastLevel;
            int lastTime = -1;
        ...
        String string = context.getString(C0444R.string.charge_length_format, new Object[]{Formatter.formatShortElapsedTime(context, this.timePeriod)});
        long j = this.remainingTimeUs;
        if (j != 0) {
            str = context.getString(C0444R.string.remaining_length_format, new Object[]{Formatter.formatShortElapsedTime(context, j / 1000)});
        } else {
            str = "";
        }
    ...
    private static void updateBatteryInfoCharging(Context context, Intent intent, BatteryUsageStats batteryUsageStats, BatteryInfo batteryInfo) {
        ...
        if (batteryInfo.isOverheated && intExtra != 5) {
            ...
            batteryInfo.chargeLabel = context.getString(C0444R.string.power_charging_limited, new Object[]{batteryInfo.batteryPercentString});
        } else if (chargeTimeRemainingMs <= 0 || intExtra == 5) {
            String batteryStatus2 = getBatteryStatus(context, intent);
            batteryInfo.remainingLabel = null;
            if (batteryInfo.batteryLevel == 100) {
                str = batteryInfo.batteryPercentString;
            } else {
                str = resources.getString(C0444R.string.power_charging, new Object[]{batteryInfo.batteryPercentString, batteryStatus2.toLowerCase()});
            }
            ...
            batteryInfo.remainingLabel = context.getString(C0444R.string.power_remaining_charging_duration_only, new Object[]{formatElapsedTime});
            batteryInfo.chargeLabel = context.getString(C0444R.string.power_charging_duration, new Object[]{batteryInfo.batteryPercentString, formatElapsedTime});
        }
    }
    ...
    private static String getBatteryStatus(Context context, Intent intent) {
        Resources resources = context.getResources();
        if (!resources.getBoolean(C0444R.bool.config_use_compact_battery_status)) {
            return Utils.getBatteryStatus(context, intent);
        }
        int intExtra = intent.getIntExtra("status", 1);
        BatteryStatus batteryStatus2 = new BatteryStatus(intent);
        String string = resources.getString(C0444R.string.battery_info_status_unknown);
        if (batteryStatus2.isCharged()) {
            return resources.getString(C0444R.string.battery_info_status_full);
        }
        if (intExtra == 2) {
            return resources.getString(C0444R.string.battery_info_status_charging);
        }
        if (intExtra == 3) {
            return resources.getString(C0444R.string.battery_info_status_discharging);
        }
        return intExtra == 4 ? resources.getString(C0444R.string.battery_info_status_not_charging) : string;
    }
