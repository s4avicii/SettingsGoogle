...
public class NotificationAccessSettings extends EmptyTextSettings {
    private static final ManagedServiceSettings.Config CONFIG = new ManagedServiceSettings.Config.Builder().setTag("NotifAccessSettings").setSetting("enabled_notification_listeners").setIntentAction("android.service.notification.NotificationListenerService").setPermission("android.permission.BIND_NOTIFICATION_LISTENER_SERVICE").setNoun("notification listener").setWarningDialogTitle(C0444R.string.notification_listener_security_warning_title).setWarningDialogSummary(C0444R.string.notification_listener_security_warning_summary).setEmptyText(C0444R.string.no_notification_listeners).build();
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C0444R.xml.notification_access_settings);
    ...
    public int getPreferenceScreenResId() {
        return C0444R.xml.notification_access_settings;
    }
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (UserManager.get(this.mContext).isManagedProfile()) {
            Toast.makeText(this.mContext, this.mDpm.getString("Settings.WORK_APPS_CANNOT_ACCESS_NOTIFICATION_SETTINGS", new NotificationAccessSettings$$ExternalSyntheticLambda2(this)), 0).show();
            finish();
        }
    }
    ...
    public /* synthetic */ String lambda$onCreate$0() throws Exception {
        return this.mContext.getString(C0444R.string.notification_settings_work_profile);
    }
    ...
    public void updateList(List<ServiceInfo> list) {
        ...
        for (ServiceInfo next : list) {
            ...
            if (managedProfileId != -10000 && !this.mDpm.isNotificationListenerServicePermitted(next.packageName, managedProfileId)) {
                appPreference.setSummary((CharSequence) this.mDpm.getString("Settings.WORK_PROFILE_NOTIFICATION_LISTENER_BLOCKED", new NotificationAccessSettings$$ExternalSyntheticLambda3(this)));
            }
    ...
    public /* synthetic */ String lambda$updateList$1() throws Exception {
        return getString(C0444R.string.work_profile_notification_access_blocked_summary);
    }
    ...
    public /* synthetic */ boolean lambda$updateList$2(ComponentName componentName, ServiceInfo serviceInfo, Preference preference) {
        ...
        new SubSettingLauncher(getContext()).setDestination(NotificationAccessDetails.class.getName()).setSourceMetricsCategory(getMetricsCategory()).setTitleRes(C0444R.string.manage_notification_access_title).setArguments(bundle).setExtras(bundle2).setUserHandle(UserHandle.getUserHandleForUid(serviceInfo.applicationInfo.uid)).launch();
        return true;
    }
