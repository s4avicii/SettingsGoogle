...
public class WifiCallingSliceHelper {
    ...
    public Slice createWifiCallingSlice(Uri uri) {
        ...
        if (!SubscriptionManager.isValidSubscriptionId(defaultVoiceSubId)) {
            ...
            return getNonActionableWifiCallingSlice(resourcesForSubId.getText(C0444R.string.wifi_calling_settings_title), resourcesForSubId.getText(C0444R.string.wifi_calling_settings_activation_instructions), uri, getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"));
        }
    }
    ...
    private Slice getWifiCallingSlice(Uri uri, boolean z, int i) {
        IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C0444R.C0447drawable.wifi_signal);
        Resources resourcesForSubId = getResourcesForSubId(i);
        return new ListBuilder(this.mContext, uri, -1).setAccentColor(Utils.getColorAccentDefaultColor(this.mContext)).addRow(new ListBuilder.RowBuilder().setTitle(resourcesForSubId.getText(C0444R.string.wifi_calling_settings_title)).addEndItem(SliceAction.createToggle(getBroadcastIntent("com.android.settings.wifi.calling.action.WIFI_CALLING_CHANGED", z), (CharSequence) null, z)).setPrimaryAction(SliceAction.createDeeplink(getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"), createWithResource, 0, resourcesForSubId.getText(C0444R.string.wifi_calling_settings_title)))).build();
    }
    ...
    public Slice createWifiCallingPreferenceSlice(Uri uri) {
        ...
        if (!isCarrierConfigManagerKeyEnabled) {
            ...
            try {
                ...
                Resources resourcesForSubId = getResourcesForSubId(defaultVoiceSubId);
                return getNonActionableWifiCallingSlice(resourcesForSubId.getText(C0444R.string.wifi_calling_mode_title), resourcesForSubId.getText(C0444R.string.wifi_calling_turn_on), uri, getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"));
    ...
    private Slice getWifiCallingPreferenceSlice(boolean z, int i, Uri uri, int i2) {
        ...
        ListBuilder.HeaderBuilder primaryAction = new ListBuilder.HeaderBuilder().setTitle(resourcesForSubId.getText(C0444R.string.wifi_calling_mode_title)).setPrimaryAction(SliceAction.createDeeplink(getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"), createWithResource, 0, resourcesForSubId.getText(C0444R.string.wifi_calling_mode_title)));
    ...
    private ListBuilder.RowBuilder wifiPreferenceRowBuilder(ListBuilder listBuilder, int i, String str, boolean z, int i2) {
        IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C0444R.C0447drawable.radio_button_check);
        Resources resourcesForSubId = getResourcesForSubId(i2);
    ...
    private Slice getNonActionableWifiCallingSlice(CharSequence charSequence, CharSequence charSequence2, Uri uri, PendingIntent pendingIntent) {
        ListBuilder.RowBuilder primaryAction = new ListBuilder.RowBuilder().setTitle(charSequence).setPrimaryAction(SliceAction.createDeeplink(pendingIntent, IconCompat.createWithResource(this.mContext, C0444R.C0447drawable.wifi_signal), 1, charSequence));
        if (!com.android.settings.Utils.isSettingsIntelligence(this.mContext)) {
