...
public final class WifiSummaryUpdater extends SummaryUpdater {
    ...
    public String getSummary() {
        ...
        if (!wifiStatusTracker.enabled) {
            return this.mContext.getString(C0444R.string.switch_off_text);
        }
        if (!wifiStatusTracker.connected) {
            return this.mContext.getString(C0444R.string.disconnected);
        }
        String sanitizeSsid = WifiInfo.sanitizeSsid(wifiStatusTracker.ssid);
        if (TextUtils.isEmpty(this.mWifiTracker.statusLabel)) {
            return sanitizeSsid;
        }
        return this.mContext.getResources().getString(C0444R.string.preference_summary_default_combination, new Object[]{sanitizeSsid, this.mWifiTracker.statusLabel});
    }
}
