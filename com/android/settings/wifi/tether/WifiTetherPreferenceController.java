...
public class WifiTetherPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnStart, OnStop {
    ...
    public void handleWifiApStateChanged(int i, int i2) {
        switch (i) {
            case 10:
                this.mPreference.setSummary((int) C0444R.string.wifi_tether_stopping);
                return;
            case 11:
                this.mPreference.setSummary((int) C0444R.string.wifi_hotspot_off_subtext);
                return;
            case 12:
                this.mPreference.setSummary((int) C0444R.string.wifi_tether_starting);
                return;
            case 13:
            ...
            default:
                if (i2 == 1) {
                    this.mPreference.setSummary((int) C0444R.string.wifi_sap_no_channel_error);
                    return;
                } else {
                    this.mPreference.setSummary((int) C0444R.string.wifi_error);
                    return;
                }
        }
    }
    ...
    private void updateConfigSummary(SoftApConfiguration softApConfiguration) {
        if (softApConfiguration != null) {
            this.mPreference.setSummary((CharSequence) this.mContext.getString(C0444R.string.wifi_tether_enabled_subtext, new Object[]{BidiFormatter.getInstance().unicodeWrap(softApConfiguration.getSsid())}));
        }
    }
}
