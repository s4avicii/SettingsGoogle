...
public class CombinedBiometricStatusPreferenceController extends BiometricStatusPreferenceController implements LifecycleObserver {
    ...
    public String getSummaryTextEnrolled() {
        return this.mContext.getString(C0444R.string.security_settings_biometric_preference_summary_none_enrolled);
    }
    ...
    public String getSummaryTextNoneEnrolled() {
        ...
        if (z && size > 1) {
            return this.mContext.getString(C0444R.string.security_settings_biometric_preference_summary_both_fp_multiple);
        }
        if (z && size == 1) {
            return this.mContext.getString(C0444R.string.security_settings_biometric_preference_summary_both_fp_single);
        }
        if (z) {
            return this.mContext.getString(C0444R.string.security_settings_face_preference_summary);
        }
        if (size <= 0) {
            return this.mContext.getString(C0444R.string.security_settings_biometric_preference_summary_none_enrolled);
        }
        return this.mContext.getResources().getQuantityString(C0444R.plurals.security_settings_fingerprint_preference_summary, size, new Object[]{Integer.valueOf(size)});
    }
