...
public class DataUsagePreference extends Preference implements TemplatePreference {
    ...
    public DataUsagePreference(Context context, AttributeSet attributeSet) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, new int[]{16843233}, TypedArrayUtils.getAttr(context, C0444R.attr.preferenceStyle, 16842894), 0);
    ...
    public void setTemplate(NetworkTemplate networkTemplate, int i, TemplatePreference.NetworkServices networkServices) {
        ...
        if (this.mTemplate.getMatchRule() == 1) {
            setTitle((int) C0444R.string.app_cellular_data_usage);
        } else {
            DataUsageController.DataUsageInfo dataUsageInfo = dataUsageController.getDataUsageInfo(this.mTemplate);
            setTitle(this.mTitleRes);
            setSummary((CharSequence) getContext().getString(C0444R.string.data_usage_template, new Object[]{DataUsageUtils.formatDataUsage(getContext(), dataUsageInfo.usageLevel), dataUsageInfo.period}));
        }
    ...
    public Intent getIntent() {
        ...
        if (this.mTemplate.getMatchRule() == 1) {
            sourceMetricsCategory.setTitleRes(C0444R.string.app_cellular_data_usage);
        } else {
