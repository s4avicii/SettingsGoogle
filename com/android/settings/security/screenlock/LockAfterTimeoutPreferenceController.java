...
public class LockAfterTimeoutPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    private void updateLockAfterPreferenceSummary(TimeoutListPreference timeoutListPreference) {
        ...
        if (timeoutListPreference.isDisabledByAdmin()) {
            str = this.mDPM.getString("Settings.DISABLED_BY_IT_ADMIN_TITLE", new LockAfterTimeoutPreferenceController$$ExternalSyntheticLambda0(this));
        } else {
            ...
            if (TextUtils.isEmpty(activeTrustAgentLabel)) {
                str = this.mContext.getString(C0444R.string.lock_after_timeout_summary, new Object[]{entries[i]});
            } else if (Long.valueOf(entryValues[i].toString()).longValue() == 0) {
                str = this.mContext.getString(C0444R.string.lock_immediately_summary_with_exception, new Object[]{activeTrustAgentLabel});
            } else {
                str = this.mContext.getString(C0444R.string.lock_after_timeout_summary_with_exception, new Object[]{entries[i], activeTrustAgentLabel});
            }
        }
