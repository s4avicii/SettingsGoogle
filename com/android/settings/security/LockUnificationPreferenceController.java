...
public class LockUnificationPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    public boolean onPreferenceChange(Preference preference, Object obj) {
        ...
        if (((Boolean) obj).booleanValue()) {
            ...
            if (!new ChooseLockSettingsHelper.Builder(this.mHost.getActivity(), this.mHost).setRequestCode(130).setTitle(this.mContext.getString(C0444R.string.unlock_set_unlock_launch_picker_title)).setReturnCredentials(true).setUserId(MY_USER_ID).show()) {
                ununifyLocks();
            }
        }
        return true;
    ...
    public void startUnification() {
        if (!new ChooseLockSettingsHelper.Builder(this.mHost.getActivity(), this.mHost).setRequestCode(129).setTitle(this.mDpm.getString("Settings.WORK_PROFILE_SET_UNLOCK_LAUNCH_PICKER_TITLE", new LockUnificationPreferenceController$$ExternalSyntheticLambda0(this))).setReturnCredentials(true).setUserId(this.mProfileUserId).show()) {
            unifyLocks();
        }
    }
    ...
    public /* synthetic */ String lambda$startUnification$0() throws Exception {
        return this.mContext.getString(C0444R.string.unlock_set_unlock_launch_picker_title_profile);
    }
    ...
    private void promptForNewDeviceLockAndThenUnify() {
        ...
        new SubSettingLauncher(this.mContext).setDestination(ChooseLockGeneric.ChooseLockGenericFragment.class.getName()).setTitleRes(C0444R.string.lock_settings_picker_title).setSourceMetricsCategory(this.mHost.getMetricsCategory()).setArguments(bundle).setTransitionType(1).launch();
    }
}
