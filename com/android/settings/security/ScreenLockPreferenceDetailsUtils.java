...
public class ScreenLockPreferenceDetailsUtils {
    ...
    public boolean isAvailable() {
        return this.mContext.getResources().getBoolean(C0444R.bool.config_show_unlock_set_or_change);
    }
    ...
    private Integer getSummaryResId(int i) {
        if (this.mLockPatternUtils.isSecure(i)) {
            ...
            if (keyguardStoredPasswordQuality == 65536) {
                return Integer.valueOf(C0444R.string.unlock_set_unlock_mode_pattern);
            }
            if (keyguardStoredPasswordQuality == 131072 || keyguardStoredPasswordQuality == 196608) {
                return Integer.valueOf(C0444R.string.unlock_set_unlock_mode_pin);
            }
            if (keyguardStoredPasswordQuality == 262144 || keyguardStoredPasswordQuality == 327680 || keyguardStoredPasswordQuality == 393216 || keyguardStoredPasswordQuality == 524288) {
                return Integer.valueOf(C0444R.string.unlock_set_unlock_mode_password);
            }
            return null;
        } else if (i == this.mProfileChallengeUserId || this.mLockPatternUtils.isLockScreenDisabled(i)) {
            return Integer.valueOf(C0444R.string.unlock_set_unlock_mode_off);
        } else {
            return Integer.valueOf(C0444R.string.unlock_set_unlock_mode_none);
        }
    }
}
