...
public class StorageWizardMoveConfirm extends StorageWizardBase {
    ...
    public void onCreate(Bundle bundle) {
        ...
        try {
            ...
            setHeaderText(C0444R.string.storage_wizard_move_confirm_title, charSequence);
            setBodyText(C0444R.string.storage_wizard_move_confirm_body, charSequence, bestVolumeDescription);
            setNextButtonText(C0444R.string.move_app, new CharSequence[0]);
        ...
        if (StorageManager.isFileEncryptedNativeOrEmulated()) {
            for (UserInfo userInfo : ((UserManager) getSystemService(UserManager.class)).getUsers()) {
                if (!StorageManager.isUserKeyUnlocked(userInfo.id)) {
                    ...
                    new ChooseLockSettingsHelper.Builder(this).setRequestCode(100).setDescription(TextUtils.expandTemplate(getText(C0444R.string.storage_wizard_move_unlock), new CharSequence[]{userInfo.name})).setUserId(userInfo.id).setForceVerifyPath(true).setAllowAnyUserId(true).show();
                    return;
                }
            }
        }
