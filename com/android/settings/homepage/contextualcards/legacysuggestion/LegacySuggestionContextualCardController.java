...
public class LegacySuggestionContextualCardController implements ContextualCardController, LifecycleObserver, OnStart, OnStop, SuggestionController.ServiceConnectionListener {
    ...
    public LegacySuggestionContextualCardController(Context context) {
        ...
        if (!context.getResources().getBoolean(C0444R.bool.config_use_legacy_suggestion)) {
    ...
    private void loadSuggestions() {
        ThreadUtils.postOnBackgroundThread((Runnable) new C0967x4d51e8b9(this));
    }
        ...
        if (suggestionController != null && this.mCardUpdateListener != null) {
            ...
            if (suggestions != null) {
                for (Suggestion next : suggestions) {
                    ...
                    builder.setPendingIntent(next.getPendingIntent()).setSuggestion(next).setName(next.getId()).setTitleText(next.getTitle().toString()).setSummaryText(next.getSummary().toString()).setViewType(C0444R.C0450layout.legacy_suggestion_tile);
    ...
    private void updateAdapter() {
        ...
        ThreadUtils.postOnMainThread(new C0968x4d51e8ba(this, arrayMap));
    }
