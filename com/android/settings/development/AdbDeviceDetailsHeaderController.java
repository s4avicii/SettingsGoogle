...
public class AdbDeviceDetailsHeaderController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver {
    ...
    private void setupEntityHeader(PreferenceScreen preferenceScreen) {
        EntityHeaderController newInstance = EntityHeaderController.newInstance(this.mFragment.getActivity(), this.mFragment, ((LayoutPreference) preferenceScreen.findPreference(KEY_HEADER)).findViewById(C0444R.C0448id.entity_header));
        this.mEntityHeaderController = newInstance;
        newInstance.setIcon(this.mContext.getDrawable(17302332)).setLabel((CharSequence) this.mPairedDevice.name).done((Activity) this.mFragment.getActivity(), true);
    }
}
