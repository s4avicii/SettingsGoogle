...
public final class PaletteListPreference extends Preference {
    ...
    public PaletteListPreference(Context context, AttributeSet attributeSet, int i) {
        ...
        setLayoutResource(C0444R.C0450layout.daltonizer_preview);
    }

    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        super.onBindViewHolder(preferenceViewHolder);
        initPaletteAttributes(getContext());
        initPaletteView(getContext(), (ViewGroup) preferenceViewHolder.itemView.findViewById(C0444R.C0448id.palette_view));
    }

    private void initPaletteAttributes(Context context) {
        int color = context.getColor(C0444R.C0446color.palette_list_gradient_background);
    ...
    private void initPaletteView(Context context, ViewGroup viewGroup) {
        ...
        float dimension = context.getResources().getDimension(C0444R.dimen.accessibility_layout_margin_start_end);
    ...
    private void updateFirstAndLastItemsBackground(Context context, ViewGroup viewGroup, int i) {
        float dimensionPixelSize = (float) context.getResources().getDimensionPixelSize(C0444R.dimen.accessibility_illustration_view_radius);
    ...
    private List<Integer> getPaletteColors(Context context) {
        return (List) Arrays.stream(context.getResources().getIntArray(C0444R.array.setting_palette_colors)).boxed().collect(Collectors.toList());
    }

    private List<String> getPaletteData(Context context) {
        return Arrays.asList(context.getResources().getStringArray(C0444R.array.setting_palette_data));
    }
