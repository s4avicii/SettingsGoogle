...
public class NrDisabledInDsdsFooterPreferenceController extends BasePreferenceController {
    ...
    private CharSequence getFooterText() {
        ...
        AnnotationSpan.LinkInfo linkInfo = new AnnotationSpan.LinkInfo(this.mContext, "url", HelpUtils.getHelpIntent(context, context.getString(C0444R.string.help_uri_5g_dsds), this.mContext.getClass().getName()));
        if (!linkInfo.isActionable()) {
            return AnnotationSpan.textWithoutLink(this.mContext.getText(C0444R.string.no_5g_in_dsds_text));
        }
        return AnnotationSpan.linkify(this.mContext.getText(C0444R.string.no_5g_in_dsds_text), linkInfo);
    }
