            ...
            for (SubscriptionInfo next : SubscriptionUtil.getAvailableSubscriptions(this.mContext)) {
                ...
                if (next.isEmbedded()) {
                    if (this.mSubscriptionManager.isActiveSubscriptionId(subscriptionId)) {
                        remove.setSummary((int) C0444R.string.mobile_network_active_esim);
                    } else {
                        remove.setSummary((int) C0444R.string.mobile_network_inactive_esim);
                    }
                } else if (this.mSubscriptionManager.isActiveSubscriptionId(subscriptionId)) {
                    remove.setSummary((int) C0444R.string.mobile_network_active_sim);
                } else if (SubscriptionUtil.showToggleForPhysicalSim(this.mSubscriptionManager)) {
                    remove.setSummary((CharSequence) this.mContext.getString(C0444R.string.mobile_network_inactive_sim));
                } else {
                    remove.setSummary((CharSequence) this.mContext.getString(C0444R.string.mobile_network_tap_to_activate, new Object[]{uniqueSubscriptionDisplayName}));
                }
