...
public class NetworkProviderCallsSmsController extends AbstractPreferenceController implements SubscriptionsChangeListener.SubscriptionsChangeListenerClient, LifecycleObserver {
    ...
    public CharSequence getSummary() {
        ...
        if (activeSubscriptions.isEmpty()) {
            return setSummaryResId(C0444R.string.calls_sms_no_sim);
        }
    ...
    public CharSequence getPreferredStatus(int i, int i2) {
        ...
        if (!SubscriptionManager.isValidSubscriptionId(i2) || !isInService(i2)) {
            return setSummaryResId(i > 1 ? C0444R.string.calls_sms_unavailable : C0444R.string.calls_sms_temp_unavailable);
        } else if (z2 && z) {
            return setSummaryResId(C0444R.string.calls_sms_preferred);
        } else {
            if (z2) {
                return setSummaryResId(C0444R.string.calls_sms_calls_preferred);
            }
            if (z) {
                return setSummaryResId(C0444R.string.calls_sms_sms_preferred);
            }
            return "";
        }
    }
