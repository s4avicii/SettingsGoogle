...
public abstract class ConfirmDeviceCredentialBaseFragment extends InstrumentedFragment {
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        this.mCancelButton = (Button) view.findViewById(C0444R.C0448id.cancelButton);
        int i = 0;
    ...
    private void setupForgotButtonIfManagedProfile(View view) {
        if (this.mUserManager.isManagedProfile(this.mUserId) && this.mUserManager.isQuietModeEnabled(UserHandle.of(this.mUserId)) && this.mDevicePolicyManager.canProfileOwnerResetPasswordWhenLocked(this.mUserId)) {
            Button button = (Button) view.findViewById(C0444R.C0448id.forgotButton);
    ...
    public void updateErrorMessage(int i) {
        ...
        if (maximumFailedPasswordsForWipe > 0 && i > 0) {
            if (this.mErrorTextView != null) {
                showError((CharSequence) getActivity().getString(C0444R.string.lock_failed_attempts_before_wipe, new Object[]{Integer.valueOf(i), Integer.valueOf(maximumFailedPasswordsForWipe)}), 0);
            }
            ...
            if (i2 <= 1) {
                ...
                if (i2 == 1) {
                    LastTryDialog.show(childFragmentManager, getActivity().getString(C0444R.string.lock_last_attempt_before_wipe_warning_title), this.mDevicePolicyManager.getString(getLastTryOverrideErrorMessageId(userTypeForWipe), new ConfirmDeviceCredentialBaseFragment$$ExternalSyntheticLambda3(this, getLastTryDefaultErrorMessage(userTypeForWipe))), 17039370, false);
                    return;
                }
                LastTryDialog.show(childFragmentManager, (String) null, getWipeMessage(userTypeForWipe), C0444R.string.lock_failed_attempts_now_wiping_dialog_dismiss, true);
            }
        }
    }
    ...
    public /* synthetic */ String lambda$getWipeMessage$3() throws Exception {
        return getString(C0444R.string.lock_failed_attempts_now_wiping_profile);
    }

    private String getWipeMessage(int i) {
        if (i == 1) {
            return getString(C0444R.string.lock_failed_attempts_now_wiping_device);
        }
        if (i == 2) {
            return this.mDevicePolicyManager.getString("Settings.WORK_PROFILE_LOCK_ATTEMPTS_FAILED", new ConfirmDeviceCredentialBaseFragment$$ExternalSyntheticLambda2(this));
        }
        if (i == 3) {
            return getString(C0444R.string.lock_failed_attempts_now_wiping_user);
        }
