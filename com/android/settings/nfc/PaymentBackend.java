...
public class PaymentBackend {
    ...
    public void setForegroundMode(boolean z) {
        for (UserHandle identifier : ((UserManager) this.mContext.createContextAsUser(UserHandle.of(UserHandle.myUserId()), 0).getSystemService(UserManager.class)).getEnabledProfiles()) {
            Settings.Secure.putIntForUser(this.mContext.getContentResolver(), "nfc_payment_foreground", z ? 1 : 0, identifier.getIdentifier());
        }
    }
    ...
    /* access modifiers changed from: package-private */
    public ComponentName getDefaultPaymentApp(int i) {
    ...
    public void setDefaultPaymentApp(ComponentName componentName, int i) {
        for (UserHandle userHandle : ((UserManager) this.mContext.createContextAsUser(UserHandle.of(ActivityManager.getCurrentUser()), 0).getSystemService(UserManager.class)).getEnabledProfiles()) {
            ...
            if (userHandle.getIdentifier() == i) {
                ...
                Settings.Secure.putStringForUser(this.mContext.getContentResolver(), "nfc_payment_default_component", (String) null, userHandle.getIdentifier());
            }
        }
        refresh();
    }
