...
public final class ThemeHelper {
    ...
    public static boolean trySetDynamicColor(Context context) {
        if (!BuildCompatUtils.isAtLeastS()) {
            LOG.mo21672w("Dynamic color require platform version at least S.");
            return false;
        } else if (!shouldApplyDynamicColor(context)) {
            LOG.mo21672w("SetupWizard does not support the dynamic color or supporting status unknown.");
            return false;
        } else {
            try {
                ...
                LOG.mo21672w("Error occurred on getting dynamic color theme.");
                return false;
            } catch (IllegalArgumentException e) {
                Logger logger = LOG;
                String message = e.getMessage();
                Objects.requireNonNull(message);
                logger.mo21668e(message);
                return false;
            }
        }
    }
