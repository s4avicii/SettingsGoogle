...
public class ReverseChargingSwitchPreferenceController extends ReverseChargingBasePreferenceController implements OnMainSwitchChangeListener {
    ...
    public void updateState(Preference preference) {
        ...
        int thresholdLevel = getThresholdLevel();
        String string = this.mContext.getString(C0444R.string.reverse_charging_switch_title);
        boolean z = true;
        if (isOverheat()) {
            string = this.mContext.getString(C0444R.string.reverse_charging_detail_charging_overheat_message);
        } else if (isPowerSaveMode()) {
            string = this.mContext.getString(C0444R.string.reverse_charging_detail_power_save_mode_is_on_message);
        } else if (this.mReverseChargingManager.isOnWirelessCharge()) {
            string = this.mContext.getString(C0444R.string.reverse_charging_detail_charging_wirelessly_message);
        } else if (this.mIsUsbPlugIn) {
            string = this.mContext.getString(C0444R.string.reverse_charging_detail_unplug_usb_cable_message);
        } else {
            int i = this.mLevel;
            if (i != NO_ERROR && i < 10) {
                string = this.mContext.getString(C0444R.string.reverse_charging_warning_title, new Object[]{Utils.formatPercentage(10)});
                Log.d(TAG, "updateState() phone is low battery ! level : " + this.mLevel);
            } else if (i != NO_ERROR && thresholdLevel >= i) {
                string = this.mContext.getString(C0444R.string.reverse_charging_warning_title, new Object[]{Utils.formatPercentage(thresholdLevel)});
