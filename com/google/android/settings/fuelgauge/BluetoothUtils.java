...
final class BluetoothUtils {
    ...
    static ContentValues wrapBluetoothData(Context context, CachedBluetoothDevice cachedBluetoothDevice, boolean z) {
        ...
        if (z) {
            int dimensionPixelSize = context.getResources().getDimensionPixelSize(C0444R.dimen.bluetooth_icon_size);
    ...
    private static void putIconMetadata(Context context, ContentValues contentValues, int i, String str, byte[] bArr) {
        ...
        if (icon != null) {
            ...
            Log.d("BluetoothUtils", String.format("putIconMetadata() for %s size=%d config=%s", new Object[]{str, Integer.valueOf(icon.getWidth()), config}));
            icon.recycle();
        }
    }
