...
public abstract class AssistGestureTrainingSliderBase extends AssistGestureTrainingBase implements SeekBar.OnSeekBarChangeListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.mErrorView = (TextView) findViewById(C0444R.C0448id.error_message);
        ...
        SeekBar seekBar = (SeekBar) findViewById(C0444R.C0448id.assist_gesture_sensitivity_seekbar);
    ...
    public void showMessage(int i, String str) {
        ...
        if (this.mErrorView.getVisibility() == 4) {
            ...
            this.mErrorView.setTranslationY((float) getResources().getDimensionPixelSize(C0444R.dimen.assist_gesture_error_text_appear_distance));
    ...
    public void clearMessage() {
        if (this.mErrorView.getVisibility() == 0) {
            this.mErrorView.animate().alpha(0.0f).translationY((float) getResources().getDimensionPixelSize(C0444R.dimen.assist_gesture_error_text_disappear_distance)).setDuration(100).setInterpolator(this.mFastOutLinearInInterpolator).withEndAction(new Runnable() {
                public void run() {
    ...
    public String getErrorString(int i) {
        if (i == 1) {
            return getResources().getString(C0444R.string.assist_gesture_training_enrolling_error_squeeze_bottom);
        }
        if (i == 2) {
            return getResources().getString(C0444R.string.assist_gesture_training_enrolling_error_squeeze_release_quickly);
        }
        if (i == 3) {
            return getResources().getString(C0444R.string.assist_gesture_training_enrolling_error_may_cause_falsing);
        }
        if (i != 4) {
            return null;
        }
        return getResources().getString(C0444R.string.assist_gesture_training_enrolling_error_try_adjusting);
    }
