...
public class ColumbusAppListPreferenceController extends BasePreferenceController implements SelectorWithWidgetPreference.OnClickListener, LifecycleObserver, OnStart {
    ...
    private final String mOpenAppValue = this.mContext.getString(C0444R.string.columbus_setting_action_launch_value);
    ...
    private void updateAppList() {
        ...
        if (preferenceCategory != null) {
            ...
            List<LauncherActivityInfo> activityList = this.mLauncherApps.getActivityList((String) null, UserHandle.of(this.mCurrentUser));
            ...
            for (LauncherActivityInfo next : activityList) {
                ArrayList arrayList = (ArrayList) queryForShortcuts.stream().filter(new ColumbusAppListPreferenceController$$ExternalSyntheticLambda2(next)).collect(Collectors.toCollection(ContextualCardManager$$ExternalSyntheticLambda8.INSTANCE));
                Bundle bundle = new Bundle();
                bundle.putParcelable(COLUMBUS_LAUNCH_APP_SECURE_KEY, next.getComponentName());
                bundle.putParcelableArrayList("columbus_app_shortcuts", arrayList);
                makeRadioPreference(next.getComponentName().flattenToString(), next.getLabel(), next.getIcon(DisplayMetrics.DENSITY_DEVICE_STABLE), arrayList.isEmpty() ? null : new ColumbusAppListPreferenceController$$ExternalSyntheticLambda0(this, bundle));
            }
        }
    }
